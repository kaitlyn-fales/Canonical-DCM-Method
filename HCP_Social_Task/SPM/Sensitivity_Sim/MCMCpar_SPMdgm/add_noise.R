...
for (i in 1:length(subjects)){
  ...
  for (j in 1:length(conditions)){
    ...
    SPM <- readMat(paste0("HCP_Social_Task/SPM/Sensitivity_Sim/MCMCpar_SPMdgm/Data/sub_",sub,"_",conditions[j],".mat"))
    BOLD <- SPM$y.signal
    u <- SPM$U
    m <- ncol(BOLD)
    ...
    save(dat, file = paste0("HCP_Social_Task/SPM/Sensitivity_Sim/MCMCpar_SPMdgm/Data/sub-",sub,"_",conditions[j],".RData"))
    
    # Write observed signal to a MATLAB file for SPM (overwrite the signal file)
    writeMat(paste0("HCP_Social_Task/SPM/Sensitivity_Sim/MCMCpar_SPMdgm/Data/sub_",sub,"_",conditions[j],".mat"),
             U = dat$u,
             Y = dat$y_obs)
...
save(SNR_df, file = "HCP_Social_Task/SPM/Sensitivity_Sim/MCMCpar_SPMdgm/Data/sim_dat_SNR.RData")
