...
# MCMC
load("HCP_Social_Task/SPM/Sensitivity_Sim/MCMCpar_SPMdgm/MCMC_results.RData")

# SPM
load("HCP_Social_Task/SPM/Sensitivity_Sim/MCMCpar_SPMdgm/SPM_results.RData")
...
for (i in 1:length(MCMC_results)){
  ...
  p <- ggplot(combined, aes(x = factor(parameter), y = mean_alpha, group = Method, colour = Method)) +
    ...
    labs(x = "", y = "") + ylim(c(-2.6,2.4)) +
    scale_color_brewer(palette = "Dark2") +
    annotate("text", x = 2.4, y = -2, label = "hat(alpha)[A]", parse = TRUE, size = 7) +
...
grid.arrange(p1, p2, p3, p4, ncol=2, nrow=2,
             top = textGrob("Canonical DCM Group-Level Parameters and SPM Data Generating Model",
