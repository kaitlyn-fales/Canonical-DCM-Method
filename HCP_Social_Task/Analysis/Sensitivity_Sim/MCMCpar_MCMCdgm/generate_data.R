...
for (i in 1:length(subjects)){
  ...
  for (j in 1:length(conditions)){
    ...
    # Posterior means
    MCMC <- MCMC_results[[conditions[j]]]
    ...
    save(dat, file = paste0("HCP_Social_Task/Analysis/Sensitivity_Sim/MCMCpar_MCMCdgm/Data/sub-",sub,"_",conditions[j],".RData"))
    
    # Write observed signal to a MATLAB file for SPM 
    writeMat(paste0("HCP_Social_Task/Analysis/Sensitivity_Sim/MCMCpar_MCMCdgm/Data/sub_",sub,"_",conditions[j],".mat"),
             U = dat$u,
             Y = dat$y_obs)
...
save(SNR_df, file = "HCP_Social_Task/Analysis/Sensitivity_Sim/MCMCpar_MCMCdgm/Data/sim_dat_SNR.RData")
