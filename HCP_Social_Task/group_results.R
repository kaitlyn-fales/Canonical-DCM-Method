...
################ Comparing between-subject SD and HPD intervals ##################
...
for (i in 1:length(MCMC_results)){
  ...
  p <- ggplot(combined, aes(x = factor(parameter), y = between_subject_sd, group = Method, colour = Method)) +
    geom_point(aes(shape = Method), size = 2.5, position = position_dodge(width = 0.6)) +
    geom_errorbar(aes(ymin = between_subject_q2.5, ymax = between_subject_q97.5, group = Method),
                  width = 0.6, position = position_dodge(width = 0.6)) +
    geom_hline(yintercept = 0, linetype = "dotted") +
    geom_vline(xintercept = c(4.5, 8.5), linetype = "dashed") +
    ggtitle(plot_titles[i]) +
    labs(x = "", y = "") + ylim(c(-0.15,1.4)) +
    scale_color_brewer(palette = "Dark2") +
    annotate("text", x = 2.5, y = -0.1, label = "hat(tau)[A]", parse = TRUE, size = 7) +
    annotate("text", x = 6.5, y = -0.1, label = "hat(tau)[B]", parse = TRUE, size = 7) +
    annotate("text", x = 9.5, y = -0.1, label = "hat(tau)[C]", parse = TRUE, size = 7) +
    theme(
      axis.text.x = element_blank(),
      axis.ticks.x = element_blank(),
      panel.background = element_blank(),
...
grid.arrange(p1, p2, p3, p4, ncol=2, nrow=2,
             top = textGrob("Group-Level Posterior Between-Subject SD Estimates and 95% HPD Intervals",
                            gp = gpar(fontface = "bold", fontsize = 16)),
             bottom = textGrob("Parameter",
                               gp = gpar(fontface = "bold", fontsize = 14)),
             left = textGrob("Group-Level Posterior Between-Subject SD",
                             gp = gpar(fontface = "bold", fontsize = 14), rot = 90))
