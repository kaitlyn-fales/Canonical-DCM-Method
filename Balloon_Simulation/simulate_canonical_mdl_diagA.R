...
save(
  true_signal,
  file = file.path(out_dir, "canonical_sim_signal_diagA.RData")
)
...
for (rep_id in seq_len(n_reps)) {
  ...
  save(dat,file = file.path(out_dir,sprintf("canonical_sim_data_diagA_rep%03d.RData", rep_id)))
  
  writeMat(file.path(out_dir,sprintf("canonical_sim_data_diagA_rep%03d.mat", rep_id)),
            U = dat$u,
            Y = dat$y_obs
  )
}
